...
class NPSNR(nn.Module):
    ...
    def forward(self, pred_ppg, labels, time):
            loss = 0
            for i in range(pred_ppg.shape[0]):
                ##############################
                # PEARSON'S CORRELATION
                ##############################
                sum_x = torch.sum(pred_ppg[i])  # x
                sum_y = torch.sum(labels[i])  # y
                sum_xy = torch.sum(pred_ppg[i] * labels[i])  # xy
                sum_x2 = torch.sum(torch.pow(pred_ppg[i], 2))  # x^2
                sum_y2 = torch.sum(torch.pow(labels[i], 2))  # y^2
                N = pred_ppg.shape[1]
                pearson = (N * sum_xy - sum_x * sum_y) / (
                    torch.sqrt((N * sum_x2 - torch.pow(sum_x, 2)) * (N * sum_y2 - torch.pow(sum_y, 2))))
                ...
                # SNR
                ##############################
                N = pred_ppg.shape[-1] * 3
                Fs = 1 / time[i].diff().mean()
                freq = torch.arange(0, N, 1, device=self.device) * Fs / N
                fft = torch.abs(torch.fft.fft(pred_ppg[i], dim=-1, n=N)) ** 2
                gt_fft = torch.abs(torch.fft.fft(labels[i], dim=-1, n=N)) ** 2
                ...
                mask = mask.masked_fill(torch.logical_and(freq < PPG_peaksLoc + (self.width / 2),
                                                          PPG_peaksLoc - (self.width / 2) < freq).to(self.device),
                                        1)  # Main signal
