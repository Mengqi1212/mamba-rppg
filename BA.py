...
class BlandAltman():
    ...
    def scatter_plot(self,x_label='Gold Standard',y_label='New Measure',
        ...
        plt.savefig(os.path.join('/home/ubuntu22/experiments/experiment1_new/user/PreTrainedModels', file_name),bbox_inches='tight', dpi=300)
    ...
    def difference_plot(self,x_label='Difference between rPPG HR and ECG HR [bpm]',
        ...
        z = gaussian_kde(xy)(xy)
        ax.scatter(avgs,diffs, c=z, label='')
        x_vals = np.array(ax.get_xlim())
        ax.axhline(self.mean_error,color='purple',label='Mean Error')
        ax.axhline(self.CI95[0],color='red',linestyle='--',label='+95% Confidence Interval')
        ax.axhline(self.CI95[1],color='red',linestyle='--',label='-95% Confidence Interval')
        ...
        if show_legend:
            ax.legend()
        ax.grid()
        plt.savefig(os.path.join('/home/ubuntu22/experiments/experiment1_new/user/PreTrainedModels/plot', file_name),bbox_inches='tight', dpi=100)
        print(f"Saved {file_name} to {self.save_path}.")
